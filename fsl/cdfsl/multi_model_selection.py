...
def train_loss_cross_validation(embeddings, y_a_i, support_size, n_support, n_way, total_epoch):
    ...
    for r in range(n_support):
        ...
        classifier_opt = torch.optim.SGD(
            net.parameters(), lr=0.01, momentum=0.9, dampening=0.9, weight_decay=0.001
        )
        ...
        for idx, embedding in enumerate(model_embeddings):
            ...
            if embeddings_all is None:
                ...
                if n_support == 20 or 50:
                    running_loss = train_loss_half_validation(
                        embedding,
                        y_a_i,
                        support_size,
                        n_support,
                    ...
                    running_loss = train_loss_cross_validation(
                        embeddings,
                        y_a_i,
                        support_size,
                        n_support,
...
def train_selection(all_embeddings, y_a_i, support_size, n_support, n_way, with_replacement=False):
    ...
    embeddings_idx_model, embeddings_all = combine_model(
        embeddings_best_of_each,
        y_a_i,
        support_size,
        n_support,
    ...
    for _, (x, y) in enumerate(novel_loader):
        ...
        if finetune_each_model:
            ...
            for idx, model_name in enumerate(pretrained_models):
                ...
                y_a_i = Variable(
                    torch.from_numpy(np.repeat(range(n_way), n_support))
                ).cuda()  # (25,)
                ...
                for epoch in range(100):
                    ...
                    for j in range(0, train_size, batch_size):
                        ...
                        selected_id = torch.from_numpy(
                            rand_id[j : min(j + batch_size, train_size)]
                        ).cuda()
        ...
        for idx, model_name in enumerate(pretrained_models):
            model_embeddings = []
            x_a_i = (
                x_var[:, :n_support, :, :, :].contiguous().view(n_way * n_support, *x.size()[2:])
            )  # (25, 3, 224, 224)
...
if __name__ == "__main__":
    ...
    for idx, novel_loader in enumerate(novel_loaders):
        ...
        test_loop(
            ...
            finetune_each_model=finetune_each_model,
            **few_shot_params
        )
